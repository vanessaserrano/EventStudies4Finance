...
AnalisisDirectorio <- function(directorioDatos,documentoEventos){
  ...
  EMPRESAS_MUESTRA <- read.table(documentoEventos,sep="\t",header=T,comment.char="",na.strings=c("#N/A","N/A"),quote="",stringsAsFactors = FALSE,dec=",")
...
ACUMULATIVA_VOLUMEN_MEDIA <- function(datos,LSPE1 = 95,LIPE1=21, LSPE2=95,LIPE2=21,LVE=10,format='%d/%m/%Y',directorio){
  ...
  #analisis_array<-read.table(analysis_array,comment.char="",na.strings=c("#N/A","N/A"),sep="\t",quote="",
  #                          header=T,dec=",")
  ...
  for(i in 1:nrow(analisis_array)){
    tabla_resultados[,b]<-VOLUMEN_MEDIA(datos=read.table(paste(directorio,'/',as.character(analisis_array[i,1]),'.txt',sep=""), comment.char="",na.strings=c("#N/A","N/A"),sep="\t",quote="",header=T,dec=","),
